    ...
    >
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 w-full relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          {/* Right side - Glass card with dev stats */}
          <div className={`hidden lg:block transition-all duration-700 delay-600 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            <div className="bg-white/5 border border-white/15 rounded-3xl p-8 shadow-2xl transition-colors duration-300" style={{ boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.08), 0 32px 64px rgba(0,0,0,0.4)' }}>
              {/* Header */}
              <div className="mb-8">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-3 h-3 rounded-full bg-blue-400"></div>
                  <span className="text-white/60 text-xs uppercase tracking-widest font-medium">{t('hero.techStack')}</span>
                </div>
                <h3 className="text-2xl font-serif text-white">{t('hero.career')}</h3>
              </div>
              ...
              <div className="space-y-4 mb-8">
                <div>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-white/80 text-sm font-medium">{t('hero.frontend')}</span>
                    <span className="text-white/50 text-xs">{t('hero.expert')}</span>
                  </div>
                  <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-blue-400 via-purple-400 to-blue-400 w-[95%] rounded-full"></div>
                  </div>
                </div>

                <div>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-white/80 text-sm font-medium">{t('hero.backend')}</span>
                    <span className="text-white/50 text-xs">{t('hero.learning')}</span>
                  </div>
                  <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-purple-400 via-blue-400 to-purple-400 w-[45%] rounded-full"></div>
                  </div>
                </div>

                {/* <div>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-white/80 text-sm font-medium">{t('hero.devops')}</span>
                    <span className="text-white/50 text-xs">{t('hero.learning')}</span>
                  </div>
                  <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-cyan-400 via-purple-400 to-cyan-400 w-[60%] rounded-full"></div>
                  </div>
                </div> */}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 pt-8 border-t border-white/5">
                <div>
                  ...
                  <p className="text-white/60 text-xs mt-1">{t('hero.yearsExp')}</p>
                </div>
                <div>
                  <div className="text-2xl font-bold text-purple-300">20+</div>
                  <p className="text-white/60 text-xs mt-1">{t('hero.projectsCompleted')}</p>
                </div>
              </div>
            </div>
          </div>
